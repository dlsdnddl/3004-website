// A방문3천사 상담신청 Google Sheets 자동 기록 + 이메일 알림 (Gmail 단일 수신자)
...
// 이메일 알림 함수 (Gmail 단일 수신자)
function sendEmailNotification(data, koreaTime) {
  // ✅ 알림 받을 Gmail 주소
  var recipient = "magudaji@gmail.com";
...
function testEmail() {
  var testData = {
    ...
    phone: "010-9999-9999",
    service_type: "테스트",
    message: "Gmail 단일 수신자 테스트입니다."
  };
  
  var now = new Date();
  var koreaTime = Utilities.formatDate(now, "Asia/Seoul", "yyyy-MM-dd HH:mm:ss");
  
  sendEmailNotification(testData, koreaTime);
  Logger.log("✅ 이메일 발송 완료! magudaji@gmail.com 확인하세요.");
}
